...
def configSelect(poolNumber: int):
    if poolNumber == 1:
        with open("/Users/georgesolorzano/ansible/ansible_learning/inside-plastics/onePool.txt", "r") as file:
            commands = file.readlines()
            commands_send = [x[:-1] for x in commands if len(x) > 1]
            return commands_send
    elif poolNumber == 4:
        with open("/Users/georgesolorzano/ansible/ansible_learning/inside-plastics/fourPool.txt", "r") as file:
            commands = file.readlines()
            commands_send = [x[:-1] for x in commands if len(x) > 1]
            return commands_send
...
def deviceconnector(i,q):
    ...
    while True:
       ...
       # print(input_commands)
        ...
        net_connect.enable()
        dhcpConfig = net_connect.send_command("show ip dhcp pool | in Pool")
        poolNumber = len(dhcpConfig.split(":\n"))
        net_connect.send_config_set(configSelect(poolNumber))
